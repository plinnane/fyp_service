...
public class NativeServiceLibrary {
	public static boolean sendMail(String subject, String body) {
		try {
			...
			try {
				addressFrom = createInternetAddress("spr-poc@ls5.cs.tu-dortmund.de", "DIME Landing Page");
			}
			...
			try {
				addressTo = createInternetAddress("spr-poc@ls5.cs.tu-dortmund.de", "The DIME Crew");
			}
			catch (UnsupportedEncodingException e) {
				// TODO logging/error handling.
				System.err.println("Unsupported encoding: " + e.getMessage());
	...
	static {
		 ...
		 */
		props.setProperty("mail.from", "spr-poc@ls5.cs.tu-dortmund.de"); // envelope sender, i.e. technical sender
		props.setProperty("mail.smtp.from", "spr-poc@ls5.cs.tu-dortmund.de"); // default "from" header, i.e. logical
		// sender
		props.setProperty("mail.smtp.host", "postamt.cs.tu-dortmund.de");
